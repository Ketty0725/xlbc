...
@Api(tags = "方剂分类介绍")
@RestController
@RequestMapping("/prescriptioncategoryintroduce")
public class PrescriptioncategoryintroduceController {
    @Autowired
    PrescriptioncategoryintroduceService service;

    @PostMapping("/getByTitle")
    public R getByTitle(String title) {
        Prescriptioncategoryintroduce bean = service.getByTitle(title);
        if (bean != null) {
