...
import static com.luck.picture.lib.immersive.RomUtils.AvailableRomType.FLYME;
import static com.luck.picture.lib.immersive.RomUtils.AvailableRomType.MIUI;
...
public class StatusBarStyle {
        ...
        try{
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                if (MIUISetStatusBarLightMode(activity, isFontDark)) {
                    ...
                    result = Build.VERSION_CODES.M;
                }else{
                    setTranslucentStatus(activity,true);
                }
            }
        }catch(Exception e){
            Log.e("","setStatusBarLightMode IS ERROR!!!");
        }
        return result;
    }
    ...
    public static void setStatusBarLightMode(Activity activity, int type) {
        ...
        } else if (type == Build.VERSION_CODES.M) {
            activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
        }
    }
